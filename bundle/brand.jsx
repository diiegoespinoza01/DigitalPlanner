...
// Estética: minimalista, crema único, serif editorial + sans limpia + mono.

// ── Tokens base (idénticos en TODAS las páginas) ──────────────────────────
const LP = {
  cream:     '#F5EFE6',
  creamDeep: '#EDE4D2',
  paper:     '#FAF6EE',
...
// ── 6 paletas Pantone suaves (mismo chroma, varía hue) ────────────────────
const THEMES = {
...
// ── CSS global (grain + clases de exportación lp-link / lp-cb) ────────────
if (typeof document !== 'undefined' && !document.getElementById('lp-base-styles')) {
  ...
  s.textContent = `
    ...
    .lp-link{cursor:pointer;transition:opacity .12s,color .12s}
...
// ── <Icon> — set de línea, viewBox 16×16, stroke 1.2 ──────────────────────
const ICON_PATHS = {
...
// ── <CB> — checkbox 14×14, marca diagonal al activarse ────────────────────
function CB({ on = false, size = 14, theme = 'greige' }) {
  const c = T(theme);
  return (
    <span className="lp-cb" style={{
      width: size, height: size, borderRadius: 3,
      border: `1px solid ${on ? c.deep : LP.line}`,
      background: on ? c.tint : 'transparent' }}>
      {on && (
        <svg width={size - 4} height={size - 4} viewBox="0 0 10 10" fill="none"
          stroke={c.deep} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
          <path d="M1.5 5.2L4 7.6 8.6 2.2" />
        </svg>
      )}
    </span>
  );
...
// ── <LinkChip> — píldora con borde fino, indica enlace ────────────────────
function LinkChip({ children, theme = 'greige', active = false, icon, style }) {
  const c = T(theme);
  return (
    <span className="lp-link" style={{
...
// ── <Eyebrow> — micro-label mono con regla hairline a la derecha ──────────
...
// ── <SideTabs> — barra vertical derecha, 5 secciones ──────────────────────
  ...
  return (
    <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width,
      ...
      {SECTIONS.map((s) => {
        const on = s === active;
        return (
          <div key={s} className="lp-link" style={{
...
// ── <TopNav> — logo serif italic + 6 chips + estado paleta ────────────────
...
function TopNav({ theme = 'greige', current, height = 58 }) {
  ...
  return (
    <div style={{ height, flex: `0 0 ${height}px`, display: 'flex', alignItems: 'center',
      ...
      <div className="lp-link lp-serif" style={{ fontStyle: 'italic', fontWeight: 500,
      ...
      <div style={{ display: 'flex', gap: 6, flex: 1 }}>
        {NAV.map((n) => {
          const on = n === current;
          return (
            <span key={n} className="lp-link" style={{
...
// ── <Page> — wrapper 1080×810: TopNav arriba + SideTabs derecha + contenido ─
function Page({ theme = 'greige', tab, currentNav, padding = 34, children, contentStyle }) {
  const tabsW = 46;
  return (
    <div className="lp-page" style={{ width: 1080, height: 810, display: 'flex', flexDirection: 'column' }}>
...
Object.assign(window, {
  LP, THEMES, THEME_ORDER, T, SECTIONS, SECTION_ICON, NAV,
